...
module.exports = class InfluxLogger extends EventEmitter {
  ...
  getRetentionPolicies() {
    ...
    return new Promise((resolve, reject) => {
      self.influx.showRetentionPolicies().then(policies => {
        let result = 'unknown'

        policies.forEach(policy => {
          if (policy.default === true) {
            result = policy.duration
            if (result === '0s') {
              result = 'Infinity'
            }
          }
  ...
  getMeasurementCount() {
    ...
    return new Promise((resolve, reject) => {
      let query = 'Select Count(value) from logging Where source=\'' + os.hostname() + '\''
      self.influx.queryRaw(query).then(rawData => {
        let cnt = 0
        if ((rawData) && (rawData.results)) {
          cnt = rawData.results[0].series[0].values[0][1]
        }
        resolve(cnt)
      })
    })
